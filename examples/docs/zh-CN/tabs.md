...
用于将关联但不同类别的内容进行分组展示，节省页面空间、切换清晰，是后台系统、表单分组、数据列表的核心组件

### 基础用法

最常用的基础标签样式，支持 `mini` / `small` / `medium` / `default` 四种尺寸；

使用 `v-model` 绑定当前激活标签，`disabled` 禁用指定标签。
...
通过 `slot="label"` 自定义标签标题，可插入图标 + 文字，视觉更直观。
...
设置 `editable` 开启新增 / 删除，适合多页签、工作 tab 场景。
