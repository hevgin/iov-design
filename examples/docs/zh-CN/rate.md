...
用于快速进行星级评价的交互组件，支持分级颜色、自定义图标、辅助文字、只读展示等功能，适用于评价、打分、满意度反馈等场景，配置灵活、展示直观。

### 基础用法

支持默认纯色展示与分级颜色展示，通过 `colors` 配置分级色彩，`low-threshold`/`high-threshold` 设定阈值，可自定义颜色分段规则。
...
只读的评分用来展示分数，允许出现半星。为组件设置 `disabled` 属性表示组件为只读，支持小数分值。此时若设置 `show-score`，则会在右侧显示目前的分值。可以提供 `score-template` 作为显示模板，模板为一个包含了 `{value}` 的字符串，`{value}` 会被解析为分值。
