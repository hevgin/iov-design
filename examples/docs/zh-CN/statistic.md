...
用于**突出展示关键数字、统计数据、金额、排名、倒计时**等核心信息，支持千分位格式化、前后缀、自定义样式，自带倒计时功能，适用于数据看板、运营统计、活动倒计时、限时抢购等场景。

倒计时模式

### 基础用法

核心能力：数值格式化、千分位分隔、精度控制、前后缀图标 / 文本、自定义展示格式；
- `value`：统计数值
...
设置 `time-indices` 即可开启倒计时模式，`value` 传入未来时间戳 / 日期对象；支持：自定义时间格式、暂停 / 继续、动态增加时间、倒计时结束回调。
- `time-indices`：开启倒计时
- `format`：时间格式（DD 天 HH 小时 mm 分钟 ss 秒）
- `@finish`：倒计时结束事件
- 实例方法 `suspend()`：暂停 / 继续

:::warning
动态追加时间时，最终时间必须大于当前时间，否则会判定为倒计时结束。
:::
:::demo 
